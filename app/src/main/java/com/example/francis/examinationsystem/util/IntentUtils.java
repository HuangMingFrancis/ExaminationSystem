...
public class IntentUtils {
    ...
    public static void openImageFile(Activity activity) {
        ...
        intent.addCategory(Intent.CATEGORY_OPENABLE);
        intent.setType("image/*");
        activity.startActivityForResult(intent, OPEN_IMGAE);
    }
    ...
    public static String openCamera(BaseFragment fragment) {
        String File_Path = Constants.Fold.PHOTO_FOLDER + fragment.getActivity().getSharedPreferences("user", 0)
                .getString("account", "") + "logo.jpg";
    ...
    public static String openCamera(Activity activity) {
        String File_Path = Constants.Fold.PHOTO_FOLDER + activity.getSharedPreferences("user", 0)
                .getString("account", "") + "logo.jpg";
        Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
        intent.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(new File(File_Path))); //文件的名字
